...
from transform import identity, lookat, lerp, perspective, sincos, vec
...
class Camera :
    """Class used for the view perspective"""
    def __init__(self, player) :
        ...
        self.distance = 11
        self.position = [0, 0, 0]
        self.target = [self.player.knight.position[0], 3 + self.player.knight.position[1], self.player.knight.position[2]]
        self.up = [0, 1, 0]
        self.viewport = [0, 0]

    def update(self, period, delta):
        self.player.update(period, delta)
        yaw_target = 180 - self.player.angle
        t = np.clip(4 * delta, 0, 1)
    ...
    def on_scroll(self, window, _deltax, deltay):
        self.distance -= 0.5 * deltay
        self.distance = np.clip(self.distance, 5 / 2 + 1, 20)
    ...
    def projection_matrix(self, winsize):
        ...
        return perspective(70, aspect, 0.5, 256)
...
class Viewer(Node):
    ...
    def set_camera(self, camera):
        self.camera = camera
        self.camera.viewport = glfw.get_framebuffer_size(self.win)
        glfw.set_scroll_callback(self.win, self.camera.on_scroll)
    ...
    def run(self):
        ...
        while not glfw.window_should_close(self.win):
            ...
            cam_pos = np.linalg.inv(self.camera.view_matrix())[:, 3].T
